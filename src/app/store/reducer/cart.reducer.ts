    ...
    switch (action.type) {
        case ECartActions.AddCartItemSuccess: {
            ...
            if (item === undefined)
                ...
                items = state.items.map(item => item.productId == action.payload? {...item, quantity: item.quantity + 1} : item)
        ...
        case ECartActions.RemoveCartItemSuccess: {
            let items = state.items.map(item => item.productId == action.payload? {...item, quantity: item.quantity - 1} : item)
